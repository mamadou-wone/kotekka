...
import { EntityArrayResponseType, UserAccessService } from '../service/user-access.service';
...
export class UserAccessComponent implements OnInit {
  ...
  trackId = (item: IUserAccess): number => this.userAccessService.getUserAccessIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.userAccesses = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IUserAccess[] | null): IUserAccess[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.userAccessService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
