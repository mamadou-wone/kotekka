...
import { EntityArrayResponseType, RecipientService } from '../service/recipient.service';
...
export class RecipientComponent implements OnInit {
  ...
  trackId = (item: IRecipient): number => this.recipientService.getRecipientIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.recipients = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IRecipient[] | null): IRecipient[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.recipientService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
