...
type RestOf<T extends IImage | NewImage> = Omit<T, 'createdDate' | 'lastModifiedDate'> & {
...
export class ImageService {
  ...
  protected convertDateFromClient<T extends IImage | NewImage | PartialUpdateImage>(image: T): RestOf<T> {
    return {
      ...image,
      createdDate: image.createdDate?.toJSON() ?? null,
      lastModifiedDate: image.lastModifiedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restImage: RestImage): IImage {
    return {
      ...restImage,
      createdDate: restImage.createdDate ? dayjs(restImage.createdDate) : undefined,
      lastModifiedDate: restImage.lastModifiedDate ? dayjs(restImage.lastModifiedDate) : undefined,
    };
  }
