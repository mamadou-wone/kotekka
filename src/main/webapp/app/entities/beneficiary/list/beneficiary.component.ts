...
import { BeneficiaryService, EntityArrayResponseType } from '../service/beneficiary.service';
...
export class BeneficiaryComponent implements OnInit {
  ...
  trackId = (item: IBeneficiary): number => this.beneficiaryService.getBeneficiaryIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.beneficiaries = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IBeneficiary[] | null): IBeneficiary[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.beneficiaryService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
